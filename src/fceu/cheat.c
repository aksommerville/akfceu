  ...
  if ((!savecheats || nosave) && !override) { /* Always save cheats if we're being overridden. */
    ...
    if (cheats) {
      ...
      if (fp) {
        for (;;) {
          struct CHEATF *t;
          if (next->type) fputc('S',fp);
          if (next->compare>=0) fputc('C',fp);

          if (!next->status) fputc(':',fp);
...
int FCEUI_GetCheat(uint32 which, char **name, uint32 *a, uint8 *v, int *compare, int *s, int *type) {
  ...
  while (next) {
    if (x==which) {
      ...
      if (v) *v=next->val;
      if (s) *s=next->status;
      if (compare) *compare=next->compare;
...
void FCEUI_CheatSearchGetRange(uint32 first, uint32 last, int (*callb)(uint32 a, uint8 last, uint8 current)) {
  ...
  for (x=0;x<0x10000;x++) {
    if (!(CheatComp[x]&CHEATC_NOSHOW) && CheatRPtrs[x>>10]) {
      if (in>=first) {
        if (!callb(x,CheatComp[x],CheatRPtrs[x>>10][x])) break;
        in++;
        if (in>last) return;
      }
    }
...
void FCEUI_CheatSearchEnd(int type, uint8 v1, uint8 v2) {
  ...
  if (type>4) FCEUI_CheatSearchSetCurrentAsOriginal();
}
